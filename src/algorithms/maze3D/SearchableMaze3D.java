...
public class SearchableMaze3D implements ISearchable {
    ...
    public ArrayList<AState> getAllSuccessors(AState curState) {
        ...
        if(columns > curColumn+1 && map[curDepth][curRow][curColumn+1] == 0)
            possibleStates.add(createMaze3DStateFromPosition3D(curDepth,curRow,curColumn+1,curState));
        //check the left cell
        ...
        if(rows > curRow+1 && map[curDepth][curRow+1][curColumn] == 0)
            possibleStates.add(createMaze3DStateFromPosition3D(curDepth,curRow+1,curColumn,curState));
        ...
        if(depth > curDepth+1 && map[curDepth+1][curRow][curColumn] == 0)
            possibleStates.add(createMaze3DStateFromPosition3D(curDepth+1,curRow,curColumn,curState));
     ...
     */
    private AState createMaze3DStateFromPosition3D(int depth, int row, int column, AState prevState){
        Maze3DState maze3DState = new Maze3DState(new Position3D(depth,row,column));
        maze3DState.setPrevState(prevState);
        return maze3DState;
    }
